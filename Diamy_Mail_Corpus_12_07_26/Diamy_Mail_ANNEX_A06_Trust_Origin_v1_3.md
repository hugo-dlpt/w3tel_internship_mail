...
**Sibling dependencies:** A01 (Inbound Gateway v1.2), A02 (Storage v1.1), A07 (Trust — Links & Attachments v1.2), A16 (Message Classification v1.3), A24 (Identity & Address Normalization v1.2)
...
| 1.3     | Jul 6th 2026 | Cédric BORNECQUE | API-relay submission observation (motivated by a real sample: sequencer-driven cold outreach submitted via the Gmail API): added the api-relay-submission path signal to §3.4 — a NEUTRAL path observation recorded with reason code `api_relay_submission`, consumed by A16 as a *nature* (classification) signal for the new `cold_outreach` class (A16-TAX-4), NOT a trust penalty; added AI error #13 (analogous to the symmetric error already noted in #12: alarming on API-relay submission alone); extended the A06-EXP-1 direction enumeration with `neutral` for recorded observations that inform sibling consumers without moving the score. Header coherence fix: sibling versions updated to current corpus (A01 v1.2, A07 v1.2, A24 v1.2 — stale since the A28/A29 batch) and A16 v1.3 added as sibling (new normative consumer of the §3.4 observation). |
...
| API-relay submission | headers — a `Received` line showing submission through a sending-API relay rather than a human MUA (e.g. `by gmailapi.google.com with HTTPREST`, ESP API endpoints). Recorded as a **neutral path observation** (reason code `api_relay_submission`, direction neutral): legitimate transactional mail is API-submitted too, so this MUST NOT contribute a trust penalty by itself (AI error #13). Its consumer is **A16** — it is a corroborating *nature* signal for the `cold_outreach` classification (A16-TAX-4), which is a kind verdict, not a danger verdict (A16-SEP-1) |
...
- **A06-EXP-1**: Every scored message MUST carry a structured explanation: an ordered list of the contributing factors, each with a direction (positive/negative/neutral — neutral exists for recorded path observations that inform sibling consumers without moving the score, e.g. `api_relay_submission`, §3.4), a severity, and a stable human-readable reason code (e.g. `dkim_valid`, `dmarc_aligned`, `domain_recently_registered`, `self_domain_dmarc_fail`, `confusable_sender_domain`, `submission_origin_anomaly`, `legacy_submission_client`, `compromised_account_pattern`, `api_relay_submission` — the last is neutral, §3.4). Reason codes are stable identifiers; their localized display strings are a client concern.
...
13. ❌ Treating the API-relay-submission observation (§3.4, `api_relay_submission`) as a trust penalty — it is a NEUTRAL path observation whose consumer is the A16 `cold_outreach` *classification* (a nature verdict), not the trust score. Legitimate transactional mail (receipts, password resets) is API-submitted; penalizing it would alarm on ordinary mail (A06-PRIN-3). The classification/trust separation (A16-SEP-1) holds in both directions.
