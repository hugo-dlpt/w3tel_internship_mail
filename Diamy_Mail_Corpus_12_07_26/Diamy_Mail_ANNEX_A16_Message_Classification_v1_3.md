...
**Sibling dependencies:** A06 (Trust — Origin v1.3), A07 (Trust — Links & Attachments v1.2), A08 (HTML→Tiptap v1.1), A01 (Inbound Gateway v1.2), A29 (Trust UX & Sandbox Workspace v1.0)
...
| 1.3     | Jul 6th 2026 | Cédric BORNECQUE | New `cold_outreach` class (motivated by a real sample: sequencer-driven B2B prospecting submitted via the Gmail API, mimicking personal 1:1 mail while carrying a campaign-token List-Unsubscribe): taxonomy row + display-label note (A16-TAX-3); three new frontier header signals in §3.1 (API-relay submission, campaign-token unsubscribe, structural contradiction); closed three-condition detection rule A16-TAX-4 (necessary campaign-token-unsubscribe marker + no-bulk-self-presentation guard + corroborant; API submission alone NEVER suffices; the guard keeps ESP newsletters with subscriber-token unsubscribes out of the class); necessary-marker false-positive discipline A16-FALSE-4; routing + visible nature-label rule A16-ROUTE-5 (out of primary inbox by default, clearly labeled as advertising, never rendered as a danger warning); test scenarios 9–11; AI errors 11–13. Sibling A06 bumped to v1.3 (records the api_relay_submission path observation this class consumes). |
...
This annex specifies **message classification**: labeling an inbound message by *kind* — marketing/bulk, cold-outreach prospecting, transactional, mailing-list, or ordinary 1:1 mail — primarily to enable a "Promotions"-style organization and to inform (but never replace) the trust assessment. It reuses signals already produced elsewhere (the Tiptap structure from A08, headers from A01, trust metadata from A06/A07), so classification is largely free once those exist.
...
| `cold_outreach` | Unsolicited commercial prospecting **disguised as personal 1:1 mail** (sequencer-driven cold outreach) | campaign-token List-Unsubscribe **without** `List-Id`/`Precedence: bulk`, personal-style Message-ID, API-relay submission (§3.1) |
...
- **A16-TAX-3** (`cold_outreach` semantics): `cold_outreach` is distinguished from `marketing` precisely by the **mimicry**: `marketing` is bulk that *presents* as bulk (list headers, promo structure, bulk domain); `cold_outreach` is bulk that *presents* as a personal 1:1 message. The mimicry is what justifies a dedicated class — the user-facing signal to produce is "this looks like a personal mail but is advertising" (§5, A16-ROUTE-5). Class identifiers are normative machine strings; display labels are localized client-side (e.g. French "Prospection"), never stored in place of the identifier.
- **A16-TAX-4** (`cold_outreach` closed detection rule — normative): The frontier base classifier assigns `cold_outreach` **if and only if** ALL THREE hold: (a) the **necessary marker** — a `List-Unsubscribe` URL carrying a campaign/tracking token (a campaign identifier and/or the recipient address embedded, plain or encoded; detectable at the frontier since the RCPT address is known); (b) the **no-bulk-self-presentation guard** — NO `List-Id` and NO `Precedence: bulk` (a message presenting as bulk is `marketing`/`mailing_list`, never `cold_outreach` — this guard is what keeps a legitimate ESP newsletter with a subscriber-token unsubscribe out of the class); (c) at least one **corroborating signal** from §3.1: API-relay submission, or a personal-style Message-ID — defined as a Message-ID whose domain is a mailbox provider's interactive-client namespace (e.g. `@mail.gmail.com`, `@*.outlook.com`) rather than the sending organization's own domain or an ESP-generator domain (the 1:1 disguise). API-relay submission ALONE — without (a) — MUST NOT trigger the class (legitimate transactional mail — receipts, password resets — is also API-submitted). The on-device history refinement (first contact / no correspondent history, A06 §7) MAY raise confidence client-side; it is NEVER required at the frontier. This rule is closed: an implementer MUST NOT add detection heuristics beyond (a)+(b)+(c) without a taxonomy version bump (A16-TAX-2).
...
| **API-relay submission** | headers — a `Received` line showing submission through a sending-API relay (e.g. `by gmailapi.google.com with HTTPREST`, ESP API endpoints); recorded by A06 §3.4 as the `api_relay_submission` path observation; a *nature* signal, not a trust penalty |
| **Campaign-token unsubscribe** | headers — `List-Unsubscribe` URL embedding a campaign identifier and/or the recipient address (plain or encoded; frontier-checkable against the RCPT address) — the sequencer signature and the `cold_outreach` necessary marker (A16-TAX-4) |
| **Structural contradiction** | headers — `List-Unsubscribe` present WITHOUT `List-Id` and WITHOUT `Precedence: bulk`, with a personal-style Message-ID — bulk compliance markers on a message dressed as personal 1:1. Decomposed in the closed rule as guard (b) + corroborant (c) (A16-TAX-4) |
...
- **A16-ROUTE-5** (`cold_outreach` routing and visible nature label — normative): A message classified `cold_outreach` (A16-TAX-4) is routed **out of the primary inbox by default** — into the Promotions view (default) or a dedicated "Prospection" view if the tenant enables one (A16-POL-1). Visibility rules are unchanged: never Trash, never silent loss (A16-ROUTE-2). The client MUST surface the class **visibly on the message itself**: a clear, contextual indication that this apparently-personal message is advertising/prospecting. This is a **nature verdict, not a danger warning** — it sits outside the trust-warning system entirely, so it MUST NOT be rendered as an alarming or trust-colored banner (rendering it as a warning would also violate A29-WARN-1, since trust warnings are signal-triggered by band and factors), and it MUST NOT alter the trust score or band in either direction (A16-SEP-1). Nor is it categorical in the A29-WARN-1 sense: it fires only on the closed header-evidence rule (A16-TAX-4), never on a blanket property like "external" or "first sender". A compliant-but-unsolicited cold outreach (aligned DMARC, valid one-click unsubscribe) keeps its normal — typically low/moderate — trust presentation alongside the nature label.
...
- **A16-FALSE-4** (`cold_outreach` necessary-marker discipline): The high-harm false positive for `cold_outreach` is labeling a **genuine human first contact** (a real person writing a first business email) as advertising. The closed rule (A16-TAX-4) makes this structurally hard: genuine personal mail carries no `List-Unsubscribe` at all, so the necessary marker is absent. A personal-looking first contact WITHOUT the campaign-token unsubscribe marker MUST be classified `personal` (its trust assessment is a separate, orthogonal question), NEVER `cold_outreach` — regardless of API submission, first-contact status, or any other soft signal.
...
9. **Sequencer cold outreach** (the motivating real sample): full SPF/DKIM/DMARC aligned pass, `Received` line via `gmailapi.google.com with HTTPREST`, `List-Unsubscribe` One-Click URL carrying a `cam_…` campaign token plus the recipient address base64-encoded, NO `List-Id`, NO `Precedence: bulk`, personal-style Message-ID → frontier base class `cold_outreach` — marker (a) ✓, guard (b) ✓, both corroborants (c) ✓ (A16-TAX-4) — routed out of the primary inbox to Promotions by default, visible "advertising/prospecting" nature label, trust band unchanged (A16-ROUTE-5, A16-SEP-1).
10. **API-submitted transactional**: a receipt or password reset emitted through an ESP API, no `List-Unsubscribe` → `transactional`, stays in the primary inbox; API-relay submission alone NEVER triggers `cold_outreach` (A16-TAX-4, A16-FALSE-4, A16-FALSE-1).
11. **Newsletter is not cold outreach**: a newsletter with `List-Id` + `List-Unsubscribe` → `mailing_list` (or `marketing`), NOT `cold_outreach` — the no-bulk-self-presentation guard (b) fails because the message presents as bulk (A16-TAX-3, A16-TAX-4). This MUST hold even when the newsletter's unsubscribe URL carries a subscriber/campaign token (marker (a) true) and it was ESP-API-submitted (corroborant (c) true) — the guard alone excludes it.
...
11. ❌ Assigning `cold_outreach` on API-relay submission alone (transactional mail is API-submitted too), without the campaign-token unsubscribe necessary marker, to a message bearing `List-Id`/`Precedence: bulk` (the guard: that is `marketing`/`mailing_list`, e.g. an ESP newsletter with a subscriber-token unsubscribe), or via heuristics beyond the closed rule (A16-TAX-4, A16-FALSE-4) — labeling a genuine human first contact as advertising is the high-harm false positive.
12. ❌ Letting the `cold_outreach` class degrade the trust score or rendering the nature label as a danger warning / alarming banner — it is a nature verdict; the classification/trust separation and the anti-categorical-banner principle both hold (A16-SEP-1, A16-ROUTE-5).
13. ❌ Requiring on-device history (first-contact status) at the frontier for `cold_outreach` detection — history only refines confidence client-side; the frontier rule is header-only (A16-TAX-4, A16-PLACE-3).
